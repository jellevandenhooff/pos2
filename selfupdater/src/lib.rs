...
impl SelfUpdater {
    pub async fn new(config: UpdaterConfiguration) -> Result<SelfUpdater> {
        Ok(Self {
    ...
    pub async fn run_helper(self, cancellation_token: CancellationToken) -> Result<()> {
        // TODO: sanity check here also?
        // make sure our id is as expected, we have access to things, ...?
        //
        self.run_update(SelfUpdater::update_from_updater_step, cancellation_token)
            .await
    }

    pub async fn start_main(self, cancellation: CancellationToken) -> Result<()> {
        ...
        let self_container = self.docker.identify_self().await?;
        let (state, _version) = self.storage.get_state()?;
        ...
        if state.status != Status::None {
            ...
            if state.old_container_id == self_container.container_id {
                self.run_update(SelfUpdater::update_from_old_step, cancellation.clone())
                    .await?;
            } else if state.new_container_id == self_container.container_id {
                self.run_update(SelfUpdater::update_from_new_step, cancellation.clone())
                    .await?;
            } else {
        ...
        // ok... now we are running successfully. run a checker in the background:
        {
            ...
            tokio::spawn(async move {
                loop {
                    if let Err(err) = self.update_loop_body(cancellation.clone()).await {
...
mod tests {
    ...
    async fn docker_idempotence() -> Result<()> {
        ...
        let test_image = "debian:trixie-slim";
