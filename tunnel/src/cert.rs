...
impl CustomHttpClient {
    fn new(tls_client_config: Arc<rustls::ClientConfig>) -> Self {
        Self(
...
fn add_certs(root_store: &mut RootCertStore, pem: &str) -> Result<()> {
    let mut certs = vec![];
    for cert in CertificateDer::pem_slice_iter(pem.as_bytes()).into_iter() {
        certs.push(cert?);
    }
    let (_ok, bad) = root_store.add_parsable_certificates(certs);
    if bad > 0 {
        bail!("bad");
    }
    Ok(())
...
pub async fn make_test_client_tlsconfig() -> Result<rustls::ClientConfig> {
...
";
    ...
    add_certs(&mut root_store, pebble_ca)?;

    let reqwest_client = reqwest::Client::builder()
        .use_rustls_tls()
        .use_preconfigured_tls(
            rustls::ClientConfig::builder()
                .with_root_certificates(Arc::new(root_store.clone()))
                .with_no_client_auth(),
        )
        .build()?;

    let fetched = reqwest_client
    ...
    add_certs(&mut root_store, &fetched)?;
    Ok(rustls::ClientConfig::builder()
        .with_root_certificates(Arc::new(root_store.clone()))
        .with_no_client_auth())
}

fn make_custom_http_client(
    rustls_client_config: Arc<rustls::ClientConfig>,
) -> Result<Box<dyn HttpClient>> {
    Ok(Box::new(CustomHttpClient::new(rustls_client_config)))
}
...
pub async fn getaccount(env: &crate::common::Environment) -> Result<Account> {
    ...
    let http_client = make_custom_http_client(env.rustls_client_config.clone())?;
