use crate::flags::{AccessFlags, LockLevel, OpenOpts, ShmLockIndex, ShmLockMode, ShmLockOp};
...
pub trait Vfs: Send + Sync {
    ...
    fn shm_lock(
        &self,
        handle: &mut Self::Handle,
        lock_idx: ShmLockIndex,
        n: usize,
        lock_type: ShmLockMode,
        lock_op: ShmLockOp,
    ) -> VfsResult<()>;
...
unsafe extern "C" fn x_shm_map<T: Vfs>(
    ...
    fallible(|| {
        let file = unwrap_file!(p_file, T)?;
        let vfs = unwrap_vfs!(file.vfs, T)?;
        let page_number: usize = i_pg.try_into().map_err(|_| vars::SQLITE_IOERR_SHMMAP)?;
        let page_size: usize = pgsz.try_into().map_err(|_| vars::SQLITE_IOERR_SHMMAP)?;
        let extend: bool = b_extend != 0;
        let mapped = vfs.shm_map(
            unsafe { file.handle.assume_init_mut() },
            page_number,
            page_size,
            extend,
        )?;
...
unsafe extern "C" fn x_shm_lock<T: Vfs>(
    ...
    fallible(|| {
        let file = unwrap_file!(p_file, T)?;
        let vfs = unwrap_vfs!(file.vfs, T)?;
        let offset: i32 = offset.try_into().map_err(|_| vars::SQLITE_IOERR_SHMMAP)?;
        let n: usize = n.try_into().map_err(|_| vars::SQLITE_IOERR_SHMMAP)?;
        let lock_op: ShmLockOp = flags.into();
        let lock_type: ShmLockMode = flags.into();
        let lock_index: ShmLockIndex = offset.into();
        vfs.shm_lock(
            unsafe { file.handle.assume_init_mut() },
            lock_index,
            n,
...
unsafe extern "C" fn x_shm_barrier<T: Vfs>(p_file: *mut ffi::sqlite3_file) {
    _ = fallible(|| {
        let file = unwrap_file!(p_file, T)?;
        let vfs = unwrap_vfs!(file.vfs, T)?;
        vfs.shm_barrier(unsafe { file.handle.assume_init_mut() });
        Ok(vars::SQLITE_OK)
    })
}
...
unsafe extern "C" fn x_shm_unmap<T: Vfs>(
    ...
    fallible(|| {
        let file = unwrap_file!(p_file, T)?;
        let vfs = unwrap_vfs!(file.vfs, T)?;
        let delete = delete_flag != 0;
        vfs.shm_unmap(unsafe { file.handle.assume_init_mut() }, delete)?;
        Ok(vars::SQLITE_OK)
    })
}
