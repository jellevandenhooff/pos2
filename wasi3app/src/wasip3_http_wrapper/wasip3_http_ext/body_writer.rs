...
use super::helpers::header_map_to_fields;
...
impl BodyWriter {
    ...
    /// Forwards the given [`http_body::Body`] to this writer.
    ///
        ...
        T::Error: Into<BoxError>,
    {
        ...
        let maybe_trailers = if self.trailers.is_empty() {
            ...
            Some(header_map_to_fields(self.trailers).map_err(Error::InvalidTrailers)?)
        };
    ...
    pub async fn process_http_body_frame<T>(&mut self, frame: Frame<T>) -> Result<usize, Error>
    where
        T: Into<Vec<u8>>,
    {
